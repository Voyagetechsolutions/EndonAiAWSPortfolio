"""Render scan results for humans and for the CI gate."""
...
def evaluate_gate(findings: list[Finding], fail_on: Severity) -> GateResult:
...
def render_console(result: GateResult) -> str:
    ...
    if not result.findings:
        lines.append("  No insecure resources found in the plan.")
    for finding in result.findings:
        blocks = ">>" if finding.severity.at_least(result.fail_on) else "  "
        target = finding.resources[0].id if finding.resources else "-"
        lines.append(f"{blocks} [{finding.severity.value:<8}] {finding.control_id:<11} {target}")
