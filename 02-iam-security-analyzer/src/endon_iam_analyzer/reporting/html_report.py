"""Self-contained HTML report, suitable for sharing or capturing as evidence."""
...
def render_html(result: AnalysisResult) -> str:
    ...
    rows = "\n".join(_row(index, finding) for index, finding in enumerate(result.by_severity()))
    empty = "" if result.findings else '<tr><td colspan="4">No findings. </td></tr>'
    return _TEMPLATE.format(
        ...
        roles=result.roles_scanned,
        policies=result.policies_scanned,
        cards=cards,
        rows=rows or empty,
    )


def _row(index: int, finding) -> str:
    color = _SEVERITY_COLORS[finding.severity.value]
    resources = "<br>".join(html.escape(r.id) for r in finding.resources)
    return f"""<tr>
...
<title>Endon AI - IAM Security Assessment</title>
<style>
  ...
  .score small {{ font-size: 15px; color: #4a5568; font-weight: 400; }}
  ...
  .scanned {{ color: #4a5568; font-size: 13px; margin-bottom: 16px; }}
  ...
  .desc {{ color: #2d3748; font-size: 13px; margin: 6px 0; }}
  .res {{ color: #4a5568; font-size: 12px; font-family: ui-monospace, monospace; }}
  .fix {{ color: #1a202c; font-size: 13px; margin-top: 6px; }}
</style>
</head>
<body>
<header>
  <h1>Endon AI &middot; IAM Security Assessment</h1>
  <div class="meta">Account {account} &middot; {region} &middot; generated {generated}</div>
</header>
<div class="wrap">
  <div class="summary">
    <div class="score">{score}<small>/100</small><div style="font-size:13px;color:#4a5568;font-weight:400">IAM security score</div></div>
    <div class="cards">{cards}</div>
  </div>
  <div class="scanned">Scanned {users} users, {roles} roles and {policies} managed policies.</div>
  <table>
