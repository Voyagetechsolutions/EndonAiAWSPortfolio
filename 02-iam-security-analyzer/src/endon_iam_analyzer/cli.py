"""Command-line entry point for the IAM analyzer.

endon-iam-analyzer scan --region us-east-1
endon-iam-analyzer scan --region us-east-1 --format html --output report.html
endon-iam-analyzer scan --profile security --formats console,json,html --output-dir reports/
"""
...
from endon_iam_analyzer.reporting import RENDERERS
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="endon-iam-analyzer",
        description="Analyze an AWS account's IAM for over-privilege and escalation paths.",
    )
    sub = parser.add_subparsers(dest="command", required=True)
    scan = sub.add_parser("scan", help="scan an account and report")
    scan.add_argument("--region", default="us-east-1")
    scan.add_argument("--profile", help="AWS named profile to use")
    scan.add_argument(
        "--formats",
        default="console",
        help="comma-separated: console, json, csv, html",
    )
...
def _scan(args: argparse.Namespace) -> int:
    ...
    snapshot = build_snapshot(clients, region=args.region)
    ...
    if args.fail_on:
        ...
        if any(f.severity.at_least(threshold) for f in result.findings):
            print(f"Findings at or above {threshold} were detected.", file=sys.stderr)
            return 1
    return 0
...
def _destination(args: argparse.Namespace, fmt: str, format_count: int) -> Path | None:
    if args.output and format_count == 1:
        return args.output
    if args.output_dir:
        return args.output_dir / f"iam-assessment.{_EXTENSIONS[fmt]}"
    return None
